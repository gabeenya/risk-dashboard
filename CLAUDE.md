...
**캐시 버스터**: `index.html`의 모든 `assets/css/*.css`, `assets/js/*.js` 참조에 `?v=YYYYMMDD<suffix>` 쿼리가 붙어 있습니다 (예: `?v=20260428a`). CSS/JS를 수정해 배포할 때마다 이 값을 일괄로 새 값으로 바꿔주세요 — 그래야 사용자 브라우저가 옛 캐시를 버리고 새 파일을 받아옵니다. 같은 날 두 번 배포하면 `a → b → c` 식으로 접미사만 올리면 됩니다.
...
- `영업비밀`, `IP`는 `SUB`에서 빈 배열로 정의되어 있어 상세 유형 선택 UI가 “해당 없음”으로 표시됩니다. (구 `IP(지식재산)`은 `IP`로 단축 — 레거시 레코드는 `loadData()`에서 자동 정규화)
